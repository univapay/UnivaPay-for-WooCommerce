...
class WC_Univapay_Gateway extends WC_Payment_Gateway {
	...
	public function process_redirect_payment() {
		...
		try {
			...
			if ( $capture || ! in_array( $payment_type, array( PaymentType::CARD(), PaymentType::PAIDY() ), true ) ) {
			...
			$this->logger->error( 'Error processing redirect payment: ' . $e->getMessage(), array( 'exception' => $e ) );
