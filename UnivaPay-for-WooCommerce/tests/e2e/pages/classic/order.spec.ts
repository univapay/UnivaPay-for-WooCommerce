...
test( 'WC Classic Checkout Inline Payment Should Be Able To Complete Order', async ( t ) => {
	...
	await univapayInlinePage.fillInlineForm( t, new MockCardData(), mockBillingData );
	...
	orderDetails.charge = charge.endsWith( '.00' )
		? charge.slice( 0, -3 )
		: charge;
} );

// eslint-disable-next-line
fixture`Univapay Console`.page`${ config.univapayConsoleUrl }`;

test( 'Univapay Console Should Receive The Same Order Request', async ( t ) => {
	if ( ! orderDetails.orderNumber || ! orderDetails.charge ) {
		throw new Error(
			'Order details are empty, checkout order has failed.'
		);
	}
	...
	await univapayConsoleTransactionsListPage.navigateToTransactionList(
		t,
		searchQuery
	);
	...
	await univapayConsoleTransactionDetailPage.confimTransactionDetail(
		t,
		orderDetails
	);
} );
